...
const addLabTest = async (req, res) => {
    try {
        const {
            ...
            is_rghs, description, lab_test_type, status, test_ids
        } = req.body;
        ...
        if (typeof test_ids === 'string') {
            try {
                ...
                parsedTestIds = test_ids.split(',').map(id => id.trim()).filter(id => id);
            }
        }
...
const updateLabTest = async (req, res) => {
    try {
        const {
            id, test_name, package_name, amount, discount, rghs_discount,
            is_rghs, description, lab_test_type, status, test_ids
        } = req.body;
        ...
        const updated = await labTestModel.updateLabTest(id, {
            ...
            is_rghs: is_rghs !== undefined
                ? (is_rghs === "true" || is_rghs === "1" || is_rghs === "Yes" || is_rghs === true)
                : existing.is_rghs,
            ...
            test_ids: parsedTestIds !== undefined ? (Array.isArray(parsedTestIds) ? parsedTestIds : []) : undefined
        });
...
const importLabTests = async (req, res) => {
    try {
        ...
        worksheet.getImages().forEach(img => {
            const imgModel = workbook.model.media.find(m => m.index === img.imageId);
            if (!imgModel) return;
        ...
        for (const item of rows) {
            ...
            const is_rghs = isRghsVal === "true" || isRghsVal === 1 || isRghsVal === true || isRghsVal === "Yes" || isRghsVal === "TRUE";
            ...
            const labTest = await labTestModel.addLabTest({
                ...
                is_rghs,
                description: getData('description') ? String(getData('description')) : null,
